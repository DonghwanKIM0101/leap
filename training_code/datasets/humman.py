...
class HuMManDataset(data.Dataset):
    ...
    def _load_data_files(self):
        ...
        with open(self.bm_path, 'rb') as smpl_file:
            smpl_model = pickle.load(smpl_file, encoding='latin1')
        self.faces = np.asarray(smpl_model['f'])
        self.sk_weights = np.asarray(smpl_model['weights'])
    ...
    def compute_sk_weights(self, vertices, sampled_points, gender=None):
        kd_tree = KDTree(vertices)
        p_idx = kd_tree.query(sampled_points)[1]
        points_sk_weights = self.sk_weights[p_idx, :]
...
class HuMManLEAPOccupancyDataset(HuMManDataset):
    """ HuMMan dataset class for occupancy training. """
...
class HuMManFWDLBSDataset(HuMManDataset):
    """ HuMMan dataset class for forward LBS training. """
...
class HuMManINVLBSDataset(HuMManDataset):
    """ HuMMan dataset class for inverse LBS training. """
